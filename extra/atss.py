    ...
    def label_anchors(self, anchors, targets):
        ...
        for im_i in range(len(targets)):
            ...
            if bboxes_per_im.shape[0]>0: 
                ...
                #print(bboxes_per_im[:, 3] - bboxes_per_im[:, 1],bboxes_per_im[:, 2] - bboxes_per_im[:, 0])
                ...
                cls_labels_per_im = torch.zeros(anchors_per_im.tensor.shape[0]).to(anchors_per_im.tensor.device)
                reg_targets_per_im = torch.zeros_like(anchors_per_im.tensor).to(anchors_per_im.tensor.device)
