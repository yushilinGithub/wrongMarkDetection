...
class DaViT(Backbone):
    ...
    def __init__(self, in_chans=3, depths=(1, 1, 3, 1), patch_size=4,
                 ...
                 drop_rate=0., attn_drop_rate=0., out_features = None,
                 ):
        super().__init__()
        ...
        for block_id, block_param in enumerate(self.architecture):
            ...
            stage = f'stage{block_id + 2}'
            if stage in self.out_features:
                self._out_feature_channels[stage] = self.embed_dims[block_id]
                self._out_feature_strides[stage] = 4 * 2 ** block_id
...
def build_davit_backbone(cfg, input_shape):
    """
    ...
    """
    model = DaViT(in_chans=input_shape.channels, 
                  depths=cfg.MODEL.DAVIT.DEPTHS, 
                  patch_size=4,
                  embed_dims=cfg.MODEL.DAVIT.EMBED_DIM, 
                  num_heads=cfg.MODEL.DAVIT.NUM_HEADS, 
                  window_size=cfg.MODEL.DAVIT.WINDOW_SIZE, 
                  mlp_ratio=cfg.MODEL.DAVIT.MLP_RATIO,
                  qkv_bias=True, 
                  drop_path_rate=cfg.MODEL.DAVIT.DROP_PATH_RATE, 
                  norm_layer=nn.LayerNorm, 
                  ...
                  drop_rate=0., 
                  attn_drop_rate=0., 
                  out_features=cfg.MODEL.DAVIT.OUT_FEATURES
                )
    if os.path.exists(cfg.MODEL.DAVIT.WEIGHTS):
        checkpoint = torch.load(cfg.MODEL.DAVIT.WEIGHTS,
            map_location="cpu")
        new_checkpoint = {}
        model_keys = model.state_dict().keys()
        for k in checkpoint["state_dict"].keys():
            if k in model_keys and model.state_dict()[k].shape == checkpoint["state_dict"][k].shape:
                
                new_checkpoint[k] = checkpoint["state_dict"][k]
            else:
                print("models {} not exist in checkpoint".format(k))
        model.load_state_dict(new_checkpoint,strict=False)
        print("load model from {}".format(cfg.MODEL.DAVIT.WEIGHTS))
    return model

@BACKBONE_REGISTRY.register()
def build_retinanet_davit_fpn_dyhead_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    """
    bottom_up = build_davit_backbone(cfg, input_shape)
    ...
    backbone = FPN(
        ...
        top_block=LastLevelP6P7(in_channels_top, out_channels, cfg.MODEL.SWINT.LAST_LEVAL_IN_FEATURE),
